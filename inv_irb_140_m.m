%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Calcula la cinematica inversa para el robot ABB IRB 140
% Entradas:
% Ver comentarios
% Salidas:
% sol = Estructura con las soluciones y las configuraciones
% Matriz de 8x6 -> 8 soluciones x 6 articulaciones
% valores articulares en radianes
% Fila de NaN cuando no hay solucion
% Matriz 8x4 -> 8 Soluciones x 4 valores de cnfgdata
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function sol = inv_irb_140_m(varargin)
% Parametros
% escala = 1;
...
switch nargin
   ...
   case 4
      ...
      robot = varargin(2);
      IRB140 = robot{1};
      qlims = cell2mat(varargin(3));
      l = cell2mat(varargin(4));
end
...
T03au = IRB140.A([1 2 3],[qinv(1,1) qinv(1,2) qinv(1,3)])*transl(0,l(4),0);
...
qinv(2,4) = q4aur;
% Wrist izquierdo
T04aul = IRB140.A([1 2 3 4],[qinv(1,1) qinv(1,2) qinv(1,3) qinv(1,4)]);
R56aul = T04aul(1:3,1:3)'*Tw(1:3,1:3);
q5aul = atan2(-R56aul(1,3),R56aul(3,3));
qinv(1,5) = q5aul;
T05aul = IRB140.A([1 2 3 4 5],[qinv(1,1) qinv(1,2) qinv(1,3) qinv(1,4) qinv(1,5)]);
R6aul = T05aul(1:3,1:3)'*Tw(1:3,1:3);
q6aul = atan2(R6aul(1,2),R6aul(3,2));
qinv(1,6) = q6aul;
% Wrist derecho
T04aur = IRB140.A([1 2 3 4],[qinv(1,1) qinv(1,2) qinv(1,3) qinv(2,4)]);
R56aur = T04aur(1:3,1:3)'*Tw(1:3,1:3);
q5aur = atan2(-R56aur(1,3),R56aur(3,3));
qinv(2,5) = q5aur;
T05aur = IRB140.A([1 2 3 4 5],[qinv(1,1) qinv(1,2) qinv(1,3) qinv(2,4) qinv(2,5)]);
R6aur = T05aur(1:3,1:3)'*Tw(1:3,1:3);
q6aur = atan2(R6aur(1,2),R6aur(3,2));
qinv(2,6) = q6aur;

% Calculo del wrist ad
T03ad = IRB140.A([1 2 3],[qinv(3,1) qinv(3,2) qinv(3,3)])*transl(0,l(4),0);
...
qinv(4,4) = q4adr;
% Wrist izquierdo
T04adl = IRB140.A([1 2 3 4],[qinv(3,1) qinv(3,2) qinv(3,3) qinv(3,4)]);
R56adl = T04adl(1:3,1:3)'*Tw(1:3,1:3);
q5adl = atan2(-R56adl(1,3),R56adl(3,3));
qinv(3,5) = q5adl;
T05adl = IRB140.A([1 2 3 4 5],[qinv(3,1) qinv(3,2) qinv(3,3) qinv(3,4) qinv(3,5)]);
R6adl = T05adl(1:3,1:3)'*Tw(1:3,1:3);
q6adl = atan2(R6adl(1,2),R6adl(3,2));
qinv(3,6) = q6adl;
% Wrist derecho
T04adr = IRB140.A([1 2 3 4],[qinv(3,1) qinv(3,2) qinv(3,3) qinv(4,4)]);
R56adr = T04adr(1:3,1:3)'*Tw(1:3,1:3);
q5adr = atan2(-R56adr(1,3),R56adr(3,3));
qinv(4,5) = q5adr;
T05adr = IRB140.A([1 2 3 4 5],[qinv(3,1) qinv(3,2) qinv(3,3) qinv(4,4) qinv(4,5)]);
R6adr = T05adr(1:3,1:3)'*Tw(1:3,1:3);
q6adr = atan2(R6adr(1,2),R6adr(3,2));
qinv(4,6) = q6adr;

% Calculo del wrist bd
T03bd = IRB140.A([1 2 3],[qinv(5,1) qinv(5,2) qinv(5,3)])*transl(0,l(4),0);
...
qinv(6,4) = q4bdr;
% Wrist izquierdo
T04bdl = IRB140.A([1 2 3 4],[qinv(5,1) qinv(5,2) qinv(5,3) qinv(5,4)]);
R56bdl = T04bdl(1:3,1:3)'*Tw(1:3,1:3);
q5bdl = atan2(-R56bdl(1,3),R56bdl(3,3));
qinv(5,5) = q5bdl;
T05bdl = IRB140.A([1 2 3 4 5],[qinv(5,1) qinv(5,2) qinv(5,3) qinv(5,4) qinv(5,5)]);
R6bdl = T05bdl(1:3,1:3)'*Tw(1:3,1:3);
q6bdl = atan2(R6bdl(1,2),R6bdl(3,2));
qinv(5,6) = q6bdl;
% Wrist derecho
T04bdr = IRB140.A([1 2 3 4],[qinv(5,1) qinv(5,2) qinv(5,3) qinv(6,4)]);
R56bdr = T04bdr(1:3,1:3)'*Tw(1:3,1:3);
q5bdr = atan2(-R56bdr(1,3),R56bdr(3,3));
qinv(6,5) = q5bdr;
T05bdr = IRB140.A([1 2 3 4 5],[qinv(5,1) qinv(5,2) qinv(5,3) qinv(6,4) qinv(6,5)]);
R6bdr = T05bdr(1:3,1:3)'*Tw(1:3,1:3);
q6bdr = atan2(R6bdr(1,2),R6bdr(3,2));
qinv(6,6) = q6bdr;

% Calculo del wrist bu
T03bu = IRB140.A([1 2 3],[qinv(7,1) qinv(7,2) qinv(7,3)])*transl(0,l(4),0);
...
qinv(8,4) = q4bur;
% Wrist izquierdo
T04bul = IRB140.A([1 2 3 4],[qinv(7,1) qinv(7,2) qinv(7,3) qinv(7,4)]);
R56bul = T04bul(1:3,1:3)'*Tw(1:3,1:3);
q5bul = atan2(-R56bul(1,3),R56bul(3,3));
qinv(7,5) = q5bul;
T05bul = IRB140.A([1 2 3 4 5],[qinv(7,1) qinv(7,2) qinv(7,3) qinv(7,4) qinv(7,5)]);
R6bul = T05bul(1:3,1:3)'*Tw(1:3,1:3);
q6bul = atan2(R6bul(1,2),R6bul(3,2));
qinv(7,6) = q6bul;
% Wrist derecho
T04bur = IRB140.A([1 2 3 4],[qinv(7,1) qinv(7,2) qinv(7,3) qinv(8,4)]);
R56bur = T04bur(1:3,1:3)'*Tw(1:3,1:3);
q5bur = atan2(-R56bur(1,3),R56bur(3,3));
qinv(8,5) = q5bur;
T05bur = IRB140.A([1 2 3 4 5],[qinv(7,1) qinv(7,2) qinv(7,3) qinv(8,4) qinv(8,5)]);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Valida que solucion cumpla limites articulares
% Entradas:
% qlim = limites articulares
% qinv = valores a validar
% Salidas:
% sol = Estructura con las soluciones y las configuraciones
% q_inv_lim = valores validados, retorna NaN en caso que no cumpla limites
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Genera la configuracion segun los cuadrantes de solucion de la inversa
% Entradas:
% qinv = Vector con los valores articulares 
% Salidas:
% cfn = Vector con los valores de articulacion
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
